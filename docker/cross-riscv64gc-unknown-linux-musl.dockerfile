...
# create soft links for compiler binaries so that cargo can find them
RUN dst="/tmp/riscv64-linux-musl-cross" && wget -O /tmp/riscv64-linux-musl-cross.tgz https://musl.cc/riscv64-linux-musl-cross.tgz && tar -xf /tmp/riscv64-linux-musl-cross.tgz -C /tmp && find "$dst/bin/" -name "riscv64-linux*" -exec sh -c 't=$(basename $1 | sed -e "s/riscv64-linux-musl/riscv64gc-unknown-linux-musl/g"); ln -s "$0/$(basename $1)" "$0/$t"' "$dst/bin" {} \; && ln -s "$dst/riscv64-linux-musl/lib/libc.so" /lib/ld-musl-riscv64.so.1
